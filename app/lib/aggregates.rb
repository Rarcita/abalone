  ...
  class Calculations
    ...
    def self.spawning_history(spawning_date, successful = %w[y Y])
      query = SpawningSuccess.not_raw
                             .where(spawning_date:
                             spawning_date, spawning_success: successful)
    ...
    # size distribution of animals within a population or
    # within the entire captive breeding program
    # => {l => c, l2 => c2, l3 => c3, ....}
    ...
    # average growth rates of tagged individuals within
    # populations or size classes over time
    # => {tag => {date => n, date2 => n2} }
    def self.growth_rates(_date_range, tags, population = nil)
      ...
      query.pluck(:tag).uniq.each do |tag|
        ...
        hash.each_with_object(0) do |(k, val), rolling_sum|
          ...
          # potential for innacurate calc if they have two
          # measurements on the same day
          raise 'Two measurements for the same day' if val.count > 1

          val = val.first
          # the .to_f is to swap this from a less readable big
          # decimal to something readable
    ...
    def self.deaths_by_cohort_for_date_range(cohort, date_range)
      query_results = MortalityTracking.not_raw
                                       .where(cohort:
                                       cohort, mortality_date: date_range)
